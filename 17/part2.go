...
func shortestPath(board [][]int, dst pos) int {
	...
	for {
		...
		var neighbors []fullPos
		if fp.lastMoveCount >= 4 {
			neighbors = []fullPos{
				fp.left(), fp.right(),
			}
		}
		if fp.lastMoveCount < 10 {
			neighbors = append(neighbors, fp.forward())
		}
		for _, p := range neighbors {
			if !p.p.valid(board) || visited[p] {
				continue
			}
...
func (p pos) valid(board [][]int) bool {
	return p.r >= 0 && p.r < len(board) && p.c >= 0 && p.c < len(board[0])
}
